...
class JCSearchResultViewController: IMBaseViewController {
    ...
    private lazy var tipsLabel: UILabel = UILabel(frame: CGRect(x: 0, y: 100, width: self.view.frame.width, height: 22.5))
    private lazy var networkErrorView: UIView = {
        let tipsView = UIView(frame: CGRect(x: 0, y: 64, width: self.view.frame.width, height: self.view.frame.height))
        var tipsLabel: UILabel = UILabel(frame: CGRect(x: 0, y: 100, width: tipsView.frame.width, height: 22.5))
        ...
        JMSGConversation.allConversations { (result, error) in
            ...
            JMSGGroup.myGroupArray { (result, error) in
                ...
                for item in result as! [NSNumber] {
                    JMSGGroup.groupInfo(withGroupId: "\(item)", completionHandler: { (result, _) in
...
//                return
//            }
//            JMSGConversation.createGroupConversation(withGroupId: group.gid) { (result, _) in
