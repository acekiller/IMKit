...
open class JCChatViewCell: UICollectionViewCell, UIGestureRecognizerDelegate {
    ...
    private func _updateViewLayouts() {
        ...
        if let view = _contentView as? UIView {
            ...
            printLog("_contentView.frame @+++> \(view.frame)")
        }
    }
    private func _updateViewValues() {
        ...
        if let view = _bubbleView {
            view.image = options.normalImage
            view.highlightedImage = options.pressImage
        }
    }
...
//                UIMenuItem(title: "撤回", action: #selector(withdrawMessage(_:))),
        ...
        self._menuNotifyObserver = NotificationCenter.default.addObserver(forName: UIMenuController.willHideMenuNotification, object: nil, queue: nil) { [weak self] _ in
            // is release?
