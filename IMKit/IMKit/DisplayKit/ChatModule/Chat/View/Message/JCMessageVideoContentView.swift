...
open class JCMessageVideoContentView: UIImageView, JCMessageContentViewType {
    ...
    open func apply(_ message: JCMessageType) {
        ...
        percentLabel.frame = CGRect(x: 0, y: 0, width: self.frame.width, height: self.frame.height)
        ...
        if content.image != nil {
            ...
            self.image = UIImage.createImage(color: UIColor(netHex: 0xCDD0D1), size: self.frame.size)
            content.videoContent?.videoThumbImageData({ (data, _, _) in
                if data != nil {
                    DispatchQueue.main.async {
        ...
        _playImageView.center = CGPoint(x: self.frame.width / 2, y: self.frame.height / 2)
    }
    ...
    @objc func _clickCell() {
        ...
        if _data != nil {
            ...
            percentLabel.textColor = .white
            weak var weakSelf = self
            content.videoContent?.videoData(progress: { (percent, _) in
                let p = Int(percent * 100)
                weakSelf?.percentLabel.text = "\(p)%"
            ...
            content.videoFileContent?.fileData(progress: { (percent, _) in
                DispatchQueue.main.async {
                    let p = Int(percent * 100)
                    weakSelf?.percentLabel.text = "\(p)%"
                }
