...
class ImageFileCell: UICollectionViewCell {
    ...
    func bindDate(_ message: JMSGMessage) {
        if message.contentType == .image {
            ...
            content.largeImageData(progress: nil, completionHandler: { (data, msgId, _) in
        ...
        content.fileData { (data, msgId, _) in
            if msgId == message.msgId {
                if let data = data {
                    let image = UIImage(data: data)
                    self.imageView.image = image
    ...
    private func _init() {
        ...
        selectView.frame = CGRect(x: self.frame.width - 31, y: 5, width: 21, height: 21)
        selectView.image = UIImage.loadImage("com_icon_file_unselect")
