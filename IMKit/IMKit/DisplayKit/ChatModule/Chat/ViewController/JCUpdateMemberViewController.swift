...
class JCUpdateMemberViewController: IMBaseViewController {
    ...
    fileprivate lazy var toolView: UIView = UIView(frame: CGRect(x: 0, y: self.topOffset, width: self.view.frame.width, height: 55))
    ...
    fileprivate lazy var data: [String: [JMSGUser]] = Dictionary()
    ...
    fileprivate lazy var tipsView: UIView = {
        let view = UIView(frame: CGRect(x: 0, y: self.topOffset + 31 + 5, width: self.view.frame.width, height: self.view.frame.height - 31 - self.topOffset - 5))
        view.backgroundColor = .white
        let tips = UILabel(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: 50))
    ...
    private func _init() {
        ...
        tableView.frame = CGRect(x: 0, y: 31 + topOffset, width: view.frame.width, height: view.frame.height - 31 - topOffset)
        ...
        searchBar.frame = CGRect(x: 15, y: 0, width: toolView.frame.width - 30, height: 31)
    ...
    fileprivate func _reloadCollectionView() {
        ...
        case 0:
            ...
            searchBar.frame = CGRect(x: 15, y: 0, width: toolView.frame.width - 30, height: 31)
            toolView.frame = CGRect(x: 0, y: topOffset, width: toolView.frame.width, height: 31)
            tableView.frame = CGRect(x: tableView.frame.origin.x, y: topOffset + 31, width: tableView.frame.width, height: view.frame.height - topOffset - 31)
            tipsView.frame = CGRect(x: 0, y: topOffset + 31 + 5, width: view.frame.width, height: view.frame.height - 31 - topOffset - 5)
        case 1:
            collectionView.frame = CGRect(x: 10, y: 0, width: 46, height: 55)
            searchBar.frame = CGRect(x: 5 + 46, y: 0, width: toolView.frame.width - 5 - 46, height: 55)
            toolView.frame = CGRect(x: 0, y: topOffset, width: toolView.frame.width, height: 55)
            tableView.frame = CGRect(x: tableView.frame.origin.x, y: topOffset + 55, width: tableView.frame.width, height: view.frame.height - topOffset - 55)
            tipsView.frame = CGRect(x: 0, y: topOffset + 31 + 5 + 24, width: view.frame.width, height: view.frame.height - 31 - 40 - 5)
        case 2:
            collectionView.frame = CGRect(x: 10, y: 0, width: 92, height: 55)
            searchBar.frame = CGRect(x: 5 + 46 * 2, y: 0, width: toolView.frame.width - 5 - 46 * 2, height: 55)
        case 3:
            collectionView.frame = CGRect(x: 10, y: 0, width: 138, height: 55)
            searchBar.frame = CGRect(x: 5 + 46 * 3, y: 0, width: toolView.frame.width - 5 - 46 * 3, height: 55)
        case 4:
            collectionView.frame = CGRect(x: 10, y: 0, width: 184, height: 55)
            searchBar.frame = CGRect(x: 5 + 46 * 4, y: 0, width: toolView.frame.width - 5 - 46 * 4, height: 55)
        default:
            collectionView.frame = CGRect(x: 10, y: 0, width: 230, height: 55)
            searchBar.frame = CGRect(x: 5 + 46 * 5, y: 0, width: toolView.frame.width - 5 - 46 * 5, height: 55)
        }
        collectionView.reloadData()
    }
    ...
    @objc func _clickNavRightButton(_ sender: UIButton) {
        ...
        if isAddMember {
            MBProgressHUDJChat.showMessage(message: "添加中...", toView: view)
            group?.addMembers(withUsernameArray: userNames, completionHandler: { (_, error) in
                MBProgressHUDJChat.hide(forView: self.view, animated: true)
                if error == nil {
                    NotificationCenter.default.post(name: Notification.Name(rawValue: kUpdateGroupInfo), object: nil)
                    self.navigationController?.popViewController(animated: true)
                } else {
                    MBProgressHUDJChat.show(text: "添加失败，请重试", view: self.view)
                }
            })
            ...
            MBProgressHUDJChat.showMessage(message: "创建中...", toView: view)
                ...
                if error == nil {
                    for vc in (self.navigationController?.viewControllers)! {
                        if vc is JCConversationListViewController {
                            ...
                            JMSGConversation.createGroupConversation(withGroupId: group.gid, completionHandler: { (_, _) in
//                                TODO: FXJ
...
//                                vc.navigationController?.pushViewController(chatVC, animated: true)
                    ...
                    MBProgressHUDJChat.show(text: "创建失败，请重试", view: self.view)
                }
            })
        }
    }
