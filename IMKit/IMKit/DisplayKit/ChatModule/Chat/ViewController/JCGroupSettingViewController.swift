...
class JCGroupSettingViewController: IMBaseViewController, CustomNavigation {
...
extension JCGroupSettingViewController: JCButtonCellDelegate {
    func buttonCell(clickButton button: UIButton) {
        let alertController = UIAlertController(title: "退出此群", message: "确定要退出此群？", preferredStyle: .alert)
    ...
    func quitGroup() {
        MBProgressHUDJChat.showMessage(message: "退出中...", toView: self.view)
...
//        }
        ...
        if actionSheet.tag == 1001 {
            if buttonIndex == 1 {
                ...
                MBProgressHUDJChat.show(text: "成功清空", view: self.view)
            }
        }
    }
}
