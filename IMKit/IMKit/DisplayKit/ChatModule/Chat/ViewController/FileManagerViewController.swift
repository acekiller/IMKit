...
class FileManagerViewController: IMBaseViewController {
    ...
    private lazy var tabedSlideView: DLTabedSlideView = {
        var tabedSlideView = DLTabedSlideView(frame: CGRect(x: 0, y: self.topOffset, width: self.view.frame.width, height: self.view.frame.height - self.topOffset))
        ...
        tabedSlideView.tabbarBackgroundImage = UIImage.createImage(color: .white, size: CGSize(width: self.view.frame.width, height: 39))
    ...
    private lazy var barView: UIView = {
        var barView = UIView(frame: CGRect(x: 0, y: self.view.frame.height - 45, width: self.view.frame.width, height: 45))
        let line = UILabel(frame: CGRect(x: 0, y: 0, width: barView.frame.width, height: 0.5))
    ...
    private func _init() {
        ...
        delButton.frame = CGRect(x: barView.frame.width - 72 - 16.6, y: 8.5, width: 72, height: 29)
        _setupNavigation()

        conversation.allMessages({ (result, _) in
    ...
    @objc func _clickNavRightButton() {
        if isEditMode {
            ...
            tabedSlideView.frame = CGRect(x: tabedSlideView.frame.origin.x, y: tabedSlideView.frame.origin.y, width: tabedSlideView.frame.width, height: tabedSlideView.frame.height + 45)
            barView.isHidden = true
        } else {
