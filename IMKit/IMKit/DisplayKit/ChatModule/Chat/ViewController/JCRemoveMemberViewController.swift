...
class JCRemoveMemberViewController: IMBaseViewController {
    ...
    fileprivate lazy var toolView: UIView = UIView(frame: CGRect(x: 0, y: 64, width: self.view.frame.width, height: 55))
    ...
    private func _init() {
        ...
        tableView.frame = CGRect(x: 0, y: 31 + 64, width: view.frame.width, height: view.frame.height - 55 - 64)
        ...
        searchView.frame = CGRect(x: 15, y: 0, width: toolView.frame.width - 30, height: 31)
    ...
    fileprivate func _reloadCollectionView() {
        ...
        case 0:
            ...
            searchView.frame = CGRect(x: 15, y: 0, width: toolView.frame.width - 30, height: 31)
            toolView.frame = CGRect(x: 0, y: 64, width: toolView.frame.width, height: 31)
            tableView.frame = CGRect(x: tableView.frame.origin.x, y: 64 + 31, width: tableView.frame.width, height: view.frame.height - 64 - 31)
        case 1:
            collectionView.frame = CGRect(x: 10, y: 0, width: 46, height: 55)
            searchView.frame = CGRect(x: 5 + 46, y: 0, width: toolView.frame.width - 5 - 46, height: 55)
            toolView.frame = CGRect(x: 0, y: 64, width: toolView.frame.width, height: 55)
            tableView.frame = CGRect(x: tableView.frame.origin.x, y: 64 + 55, width: tableView.frame.width, height: view.frame.height - 64 - 55)
        case 2:
            collectionView.frame = CGRect(x: 10, y: 0, width: 92, height: 55)
            searchView.frame = CGRect(x: 5 + 46 * 2, y: 0, width: toolView.frame.width - 5 - 46 * 2, height: 55)
        case 3:
            collectionView.frame = CGRect(x: 10, y: 0, width: 138, height: 55)
            searchView.frame = CGRect(x: 5 + 46 * 3, y: 0, width: toolView.frame.width - 5 - 46 * 3, height: 55)
        case 4:
            collectionView.frame = CGRect(x: 10, y: 0, width: 184, height: 55)
            searchView.frame = CGRect(x: 5 + 46 * 4, y: 0, width: toolView.frame.width - 5 - 46 * 4, height: 55)
        default:
            collectionView.frame = CGRect(x: 10, y: 0, width: 230, height: 55)
            searchView.frame = CGRect(x: 5 + 46 * 5, y: 0, width: toolView.frame.width - 5 - 46 * 5, height: 55)
        }
        collectionView.reloadData()
    }
    ...
    @objc func _clickNavRightButton(_ sender: UIButton) {
        ...
        MBProgressHUDJChat.showMessage(message: "删除中...", toView: view)
        group.removeMembers(withUsernameArray: userNames) { (_, error) in
            ...
            if error == nil {
                ...
                MBProgressHUDJChat.show(text: "删除失败，请重试", view: self.view)
            }
        }
    }
