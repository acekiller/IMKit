...
/// 聊天列表
public class JCConversationListViewController: IMBaseViewController {
    ...
    public override func viewWillAppear(_ animated: Bool) {
        _ = IMChatManger.shared.loginIM().subscribe(onNext: { _ in
            print("logined")
        }, onError: { _ in
...
//    private lazy var searchView: UIView = UIView(frame: CGRect(x: 0, y: 0, width: self.view.width, height: 36))
    fileprivate lazy var tableview: UITableView = {
        var tableview = UITableView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height))
        tableview.delegate = self
        tableview.dataSource = self
        tableview.backgroundColor = .white
        tableview.register(JCConversationCell.self, forCellReuseIdentifier: "JCConversationCell")
        tableview.separatorStyle = .none
        tableview.backgroundColor = UIColor(netHex: 0xF5F7FC)
        return tableview
    }()
    ...
    fileprivate lazy var emptyView: UIView = {
        ...
        view.isHidden = true
        view.backgroundColor = UIColor(netHex: 0xF5F7FC)
        let tips = UIImageView()
        tips.contentMode = .scaleAspectFit
        tips.image = ThemeManager.instance.loadImage("chatNoMessageEmpty", .default)
        tips.frame = CGRect(x: 0, y: 32, width: 160, height: 160)
        tips.tz_centerX = self.view.tz_centerX
        view.addSubview(tips)
        let tost = UILabel()
        tost.text = "暂无消息"
        tost.font = .systemFont(ofSize: 16)
        tost.textColor = .black
        tost.sizeToFit()
        view.addSubview(tost)
        tost.snp.makeConstraints { make in
    ...
    fileprivate lazy var titleTipsView: UIView = {
        var view = UIView(frame: CGRect(x: self.view.frame.width / 2 - 45, y: 20, width: 90, height: 44))
...
//    }
    ...
    fileprivate lazy var selectView: YHPopupView = {
        let popupView = MorePopupView(frame: CGRect(x: self.view.frame.width - 150, y: 65, width: 145, height: 554 / 3))
...
extension JCConversationListViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        let action1 = UITableViewRowAction(style: .destructive, title: "删除") { (_, indexPath) in
            self._delete(indexPath)
        }
        let conversation = datas[showNetworkTips ? indexPath.row - 1 : indexPath.row]
        let action2 = UITableViewRowAction(style: .normal, title: "置顶") { (_, _) in
...
extension JCConversationListViewController: UISearchControllerDelegate {
    public func willPresentSearchController(_ searchController: UISearchController) {
        ...
        UIView.animate(withDuration: 0.35, animations: {
            self.emptyView.frame = CGRect(x: 0, y: 64, width: self.view.frame.width, height: self.view.frame.height - 64)
        }) { (_) in
    ...
    public func willDismissSearchController(_ searchController: UISearchController) {
        UIView.animate(withDuration: 0.35) {
            self.emptyView.frame = CGRect(x: 0, y: 64 + 36, width: self.view.frame.width, height: self.view.frame.height - 64 - 36)
        }
