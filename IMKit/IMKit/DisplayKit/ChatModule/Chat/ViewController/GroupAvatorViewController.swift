...
class GroupAvatorViewController: IMBaseViewController {
...
// Local variable inserted by Swift 4.2 migrator.
        ...
        if image != nil {
            MBProgressHUDJChat.showMessage(message: "正在上传", toView: view)
            ...
            JMSGGroup.updateInfo(withGid: group.gid, groupInfo: info, completionHandler: { (_, error) in
                DispatchQueue.main.async(execute: { () -> Void in
                    MBProgressHUDJChat.hide(forView: self.view, animated: true)
                    if error == nil {
                        MBProgressHUDJChat.show(text: "上传成功", view: self.view)
                        self.imageView.image = image
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: kUpdateGroupInfo), object: nil)
                    } else {
                        MBProgressHUDJChat.show(text: "上传失败", view: self.view)
                    }
                })
            })
        }
