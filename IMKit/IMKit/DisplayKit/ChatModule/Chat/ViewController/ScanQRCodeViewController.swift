...
class ScanQRCodeViewController: IMBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        let borderView = UIImageView(frame: CGRect(x: (view.frame.width - 240) / 2, y: 123, width: 240, height: 240))
        borderView.image = UIImage.loadImage("com_icon_qrc_border")
        view.addSubview(borderView)

        qrcLine = UIImageView(frame: CGRect(x: (view.frame.width - 190) / 2, y: 123, width: 190, height: 6))
        ...
        let tipsLabel = UILabel(frame: CGRect(x: 0, y: 123 + 240 + 17.5, width: view.frame.width, height: 20))
    ...
    private func _getBackgroundImage() -> UIImage? {
        ...
        ctx.fill(drawRect)
        drawRect = CGRect(x: (view.frame.width - 240) / 2, y: 123, width: 240, height: 240)
        ctx.clear(drawRect)
    ...
    @objc func startQRCAnimate() {
        ...
        isAnimating = true
        qrcLine.frame = CGRect(x: (self.view.frame.width - 190) / 2, y: 123, width: 190, height: 6)
        UIView.animate(withDuration: 2.5, animations: {
            self.qrcLine.frame = CGRect(x: (self.view.frame.width - 190) / 2, y: 123 + 240 - 5, width: 190, height: 6)
        }) { (finish) in
            self.isAnimating = false
            self.qrcLine.frame = CGRect(x: (self.view.frame.width - 190) / 2, y: 123, width: 190, height: 6)
            if finish {
...
extension ScanQRCodeViewController: AVCaptureMetadataOutputObjectsDelegate {
    func metadataOutput(_ captureOutput: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        for metadataObject in metadataObjects {
            ...
            let validFrame = CGRect(x: (view.frame.width - 240) / 2, y: 123, width: 240, height: 240)
            ...
            if object.type.rawValue == convertFromAVMetadataObjectObjectType(AVMetadataObject.ObjectType.qr) {
                ...
                session.stopRunning()
                JMSGUser.userInfoArray(withUsernameArray: [username], appKey: appkey, completionHandler: { (_, error) in
                    if error == nil {
//                        TODO: FXJ
