...
class JCSingleSettingViewController: IMBaseViewController, CustomNavigation {
    ...
    private lazy var tableView: UITableView = {
        var tableView = UITableView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height), style: .grouped)
    ...
    func actionSheet(_ actionSheet: UIActionSheet, clickedButtonAt buttonIndex: Int) {
        ...
        if actionSheet.tag == 1001 {
            if buttonIndex == 1 {
                ...
                MBProgressHUDJChat.show(text: "成功清空", view: view)
            }
        }
    }
...
extension JCSingleSettingViewController: JCMineInfoCellDelegate {
    func mineInfoCell(clickSwitchButton button: UISwitch, indexPath: IndexPath?) {
        if user.isNoDisturb != button.isOn {
            MBProgressHUDJChat.showMessage(message: "修改中", toView: view)
            user.setIsNoDisturb(button.isOn, handler: { (_, error) in
                MBProgressHUDJChat.hide(forView: self.view, animated: true)
                if error == nil {
                    MBProgressHUDJChat.show(text: "修改成功", view: self.view)
                } else {
                    MBProgressHUDJChat.show(text: "修改失败", view: self.view)
                }
            })
        }
    }
...
extension JCSingleSettingViewController: UIAlertViewDelegate {
    func alertView(_ alertView: UIAlertView, clickedButtonAt buttonIndex: Int) {
        if buttonIndex == 1 {
            JMSGFriendManager.removeFriend(withUsername: user.username, appKey: user.appKey, completionHandler: { (_, error) in
                if error == nil {
                    ...
                    MBProgressHUDJChat.show(text: "\(String.errorAlert(error! as NSError))", view: self.view)
                }
            })
        }
    }
