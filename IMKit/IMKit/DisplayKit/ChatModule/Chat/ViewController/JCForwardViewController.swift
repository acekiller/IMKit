...
class JCForwardViewController: IMBaseViewController {
    ...
    fileprivate lazy var contacterView: UITableView = {
        ...
        contacterView.frame = CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height)
    ...
    private lazy var searchView: UIView = UIView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 31))
    ...
    private func _init() {
        ...
        var vc: JCSearchResultViewController!
        if nav.topViewController is RTContainerController {
            vc = (nav.topViewController as! RTContainerController).contentViewController as? JCSearchResultViewController
        } else {
            vc = nav.topViewController as? JCSearchResultViewController
        }
    ...
    func _getFriends() {
        JMSGFriendManager.getFriendList { (result, _) in
...
extension JCForwardViewController: UIAlertViewDelegate {
    func alertView(_ alertView: UIAlertView, clickedButtonAt buttonIndex: Int) {
        ...
        case 10003:
            JMSGConversation.createSingleConversation(withUsername: selectUser.username) { (result, _) in
        ...
        MBProgressHUDJChat.show(text: "已发送", view: view, 2)
