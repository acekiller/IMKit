...
class UnreadListViewController: IMBaseViewController, CustomNavigation {
    ...
    private lazy var tabedSlideView: DLTabedSlideView = {
        var tabedSlideView = DLTabedSlideView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height - 0))
    ...
    private func _init() {
        ...
        MBProgressHUDJChat.showMessage(message: "获取中...", toView: view)
        message.messageReadDetailHandler { (readUsers, unreadUsers, error) in
            ...
            if error == nil {
                ...
                MBProgressHUDJChat.show(text: "获取失败", view: self.view)
            }
        }
...
class UnreadListTableView: UITableViewController {
    ...
    private func _init() {
        ...
        let line = UILabel(frame: CGRect(x: 0, y: 0, width: view.frame.width, height: 0.5))
