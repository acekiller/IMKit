...
open class SAIInputBar: UIView {
    open var adaptInputBarHandle: (SAIInputBar, SAIInputMode) -> Void = { _, _ in }
    ...
    open var inputMode: SAIInputMode {
        set {
            _updateInputMode(newValue, animated: true)
        }
    ...
    fileprivate func _updateInputMode(_ newMode: SAIInputMode, animated: Bool) {
        ...
        adaptInputBarHandle(self, newMode)
    }
    fileprivate func _updateInputModeForResponder(_ newMode: SAIInputMode, animated: Bool) {
        ...
        _inputMode = newMode
        _updateInputMode(newMode, animated: animated)
        _inputView.updateInputMode(newMode, oldMode: oldMode, animated: animated)
...
//        let color = UIColor(colorLiteralRed: 0xec / 0xff, green: 0xed / 0xff, blue: 0xf1 / 0xff, alpha: 1)
    ...
    fileprivate(set) public lazy var _inputAccessoryView: SAIInputAccessoryView = SAIInputAccessoryView()
    fileprivate(set) public lazy var _backgroundView: SAIInputBackgroundView = SAIInputBackgroundView()
...
//            _updateSystemKeyboard(ef1.size, animated: false)
    ...
    private func _ntf_flatMap(_ ntf: Notification, handler: (CGRect, CGRect, TimeInterval, UIView.AnimationCurve) -> Void) {
...
extension SAIInputBar {
    ...
    open func setBarItem(_ barItem: SAIInputItem, atPosition position: SAIInputItemPosition, animated: Bool = false) {
        return _inputAccessoryView.setBarItems([barItem], atPosition: position, animated: animated)
    }
    open func setBarItems(_ barItems: [SAIInputItem], atPosition position: SAIInputItemPosition, animated: Bool = false) {
