...
class JCButtonCell: UITableViewCell {
    ...
    var buttonColor: UIColor {
        ...
        set {
            color = newValue
            button.setBackgroundImage(UIImage.createImage(color: color, size: CGSize(width: self.contentView.frame.width - 30, height: self.contentView.frame.height)), for: .normal)
        }
    }
    ...
    private func _init() {
        ...
        button.setBackgroundImage(UIImage.createImage(color: color, size: CGSize(width: contentView.frame.width - 30, height: contentView.frame.height)), for: .normal)
